...
export async function POST(request) {
  const { email, num, name, country, city, address, products, slipimg, paymentMethod } = await request.json();
  ...
  // Create a new checkout record with the new serial number
  ...
  return NextResponse.json({ message: "Checkout Created", num: newSerialNumber }, { status: 201 });
}
...
export async function DELETE(request) {
  ...
  return NextResponse.json({ message: "Checkout deleted" }, { status: 200 });
}
